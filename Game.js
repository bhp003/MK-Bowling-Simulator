module.exports = class Game {
  ...
  play() {
    ...
    for (let frame = 1; frame <= this.totalFrames; frame++) {
      ...
      for (let bowl = 1; bowl <= this.bowlPerFrame; bowl++) {
        const score = maxScorePerBowl - Math.floor(Math.random() * (maxScorePerBowl + 1));
    ...
    if ((lastFrame[0] > 0 || lastFrame[1] > 0) 
      ...
      const extraBowlScore = Math.floor(Math.random() * this.maxScorePerBowl + 1);
  ...
  recordRolls() {
    ...
    this.frames[this.totalFrames - 1].forEach(score => this.rolls.push(score));
  }
  ...
  getScores() {
    ...
    for (let frame = 0; frame < this.totalFrames; frame++) {
      if (this.isStrike(index)) {
        score += this.maxScorePerBowl + this.rolls[index + 1] + this.rolls[index + 2];
        index++;
      }
